...
#define LINE_IS(text) !_strnicmp((buf), (text), strlen(text))
...
static bool write_obj(TriMesh *mesh, FILE *f);
...
bool TriMesh::write(const char *filename)
{
	...
	if (c) {
		if (!_strnicmp(c, ".ply", 4))
			filetype = we_are_little_endian() ?
					PLY_BINARY_LE :
					PLY_BINARY_BE;
		else if (!_strnicmp(c, ".ray", 4))
			filetype = RAY;
		else if (!_strnicmp(c, ".obj", 4))
			filetype = OBJ;
		else if (!_strnicmp(c, ".off", 4))
			filetype = OFF;
		else if (!_strnicmp(c, ".sm", 3))
			filetype = SM;
		else if (!_strnicmp(c, ".cc", 3))
			filetype = CC;
		else if (!_strnicmp(c, ".c++", 4))
			filetype = CC;
		else if (!_strnicmp(c, ".cpp", 4))
			filetype = CC;
		else if (!_strnicmp(c, ".C", 2))
			filetype = CC;
	}

	// Handle filetype:filename.foo constructs
	while (1) {
		if (!_strnicmp(filename, "norm:", 5)) {
			filename += 5;
			write_norm = true;
		} else if (!_strnicmp(filename, "nogrid:", 7)) {
			filename += 7;
			write_grid = false;
		} else if (!_strnicmp(filename, "cflt:", 5)) {
			filename += 5;
			float_color = true;
		} else if (!_strnicmp(filename, "ply:", 4)) {
			filename += 4;
			filetype = we_are_little_endian() ?
					PLY_BINARY_LE :
					PLY_BINARY_BE;
		} else if (!_strnicmp(filename, "ply_binary:", 11)) {
			filename += 11;
			filetype = we_are_little_endian() ?
					PLY_BINARY_LE :
					PLY_BINARY_BE;
		} else if (!_strnicmp(filename, "ply_binary_be:", 14)) {
			filename += 14;
			filetype = PLY_BINARY_BE;
		} else if (!_strnicmp(filename, "ply_binary_le:", 14)) {
			filename += 14;
			filetype = PLY_BINARY_LE;
		} else if (!_strnicmp(filename, "ply_ascii:", 10)) {
			filename += 10;
			filetype = PLY_ASCII;
		} else if (!_strnicmp(filename, "ply_asc:", 8)) {
			filename += 8;
			filetype = PLY_ASCII;
		} else if (!_strnicmp(filename, "ascii:", 6)) {
			filename += 6;
			filetype = PLY_ASCII;
		} else if (!_strnicmp(filename, "asc:", 4)) {
			filename += 4;
			filetype = PLY_ASCII;
		} else if (!_strnicmp(filename, "be:", 3)) {
			filename += 3;
			filetype = PLY_BINARY_BE;
		} else if (!_strnicmp(filename, "le:", 3)) {
			filename += 3;
			filetype = PLY_BINARY_LE;
		} else if (!_strnicmp(filename, "ray:", 4)) {
			filename += 4;
			filetype = RAY;
		} else if (!_strnicmp(filename, "obj:", 4)) {
			filename += 4;
			filetype = OBJ;
		} else if (!_strnicmp(filename, "off:", 4)) {
			filename += 4;
			filetype = OFF;
		} else if (!_strnicmp(filename, "sm:", 3)) {
			filename += 3;
			filetype = SM;
		} else {
			break;
...
{
	FPRINTF(f, "# OBJ\n");
	if (!write_verts_asc(mesh, f, "v ", 0, 0, false, 0, ""))
		return false;
	mesh->need_faces();
	for (size_t i = 0; i < mesh->faces.size(); i++) {
		mesh->faces[i][0]++;
	...
	bool ok = write_faces_asc(mesh, f, "f ", "");
	for (size_t i = 0; i < mesh->faces.size(); i++) {
		mesh->faces[i][0]--;
		mesh->faces[i][1]--;
		mesh->faces[i][2]--;
